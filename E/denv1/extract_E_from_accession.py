...
def search_and_download_e_proteins(accession_ids, email, batch_size=20):
    ...
    print(f"Searching for E protein sequences for {len(accession_ids)} DENV1 genomes...")
    ...
    for i in range(0, len(accession_ids), batch_size):
        ...
        try:
            ...
            # Filter for DENV1 E proteins
            batch_e_proteins = []
            for record in protein_records:
                desc = record.description.lower()

                # Check if it's DENV1 E protein
                is_denv1 = ('denv-1' in desc or 'dengue virus 1' in desc or
                            'dengue virus type 1' in desc or 'denv1' in desc)
                ...
                if is_denv1 and is_e_protein and length_ok:
...
def direct_protein_access(accession_ids, email):
    ...
    # Search for DENV1 envelope proteins
    search_term = '(dengue virus 1[Organism] OR DENV-1[All Fields]) AND ' \
                  '(envelope[Title] OR glycoprotein E[Title] OR E protein[Title])'

    print("Searching for DENV1 envelope proteins in NCBI Protein database...")
...
def main():
    ...
    # Read accession IDs
    with open("DENV1_accession_ids.txt", "r") as f:
        accession_ids = [line.strip() for line in f if line.strip()]

    print(f"Loaded {len(accession_ids)} DENV1 accession IDs")
    ...
    if e_proteins:
        ...
        SeqIO.write(e_proteins, "DENV1_E_proteins_direct_search.fasta", "fasta")
        print(f"\n✅ Saved {len(e_proteins)} E proteins to DENV1_E_proteins_direct_search.fasta")

        # Filter for complete ones (~495 aa)
        complete_e = [rec for rec in e_proteins if 480 <= len(rec.seq) <= 510]
        if complete_e:
            SeqIO.write(complete_e, "DENV1_E_proteins_complete.fasta", "fasta")
            print(f"✅ Saved {len(complete_e)} complete E proteins (480-510 aa)")

        # Create a summary
        with open("DENV1_E_proteins_summary.txt", "w") as f:
    ...
    if linked_e_proteins:
        SeqIO.write(linked_e_proteins, "DENV1_E_proteins_linked.fasta", "fasta")
